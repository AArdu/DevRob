...
<<<<<<< HEAD
    ...
    motion_p.angleInterpolation(["HeadYaw", "HeadPitch"], [
                                alpha, beta], [1.5, 1.5], False)


def follow_gaze(cam, GazeNet, faceInfo):
    img = get_remote_image(cam)
    fd = FaceDetection.FaceDetector(img, True)
    ...
    look_at_gazed(gaze_coords, cam)


def look_at_gazed(coords, cam):
    # get the angles needed to look at the pointed object
    # TODO: get head angles given the direction of the gaze
    ...
    if joint_angles == '':
        ...
        motion_p.positionInterpolations(
            "Head", frame, joint_angles[:5], 7, 1.5)
...
<<<<<<< HEAD
    try:
        ...
        # QUESTION: do we need matlab?
        # matlab=subprocess.Popen(
        #     ["C:\\Users\\univeristy\\Anaconda3\\python.exe", "py64environment.py"])
        GazeNet=GNet()
        ...
        while True:
            ...
            angle_list=[list(np.random.uniform(-0.8, 0.8, 1)),
                             list(np.random.uniform(-0.6, 0.6, 1))]
            times=[[1.25], [1.25]]
        ...
        follow_gaze(cam, GazeNet, faceInfo)
